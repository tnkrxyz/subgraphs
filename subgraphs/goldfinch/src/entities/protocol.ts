...
import { _Protocol } from "../../generated/schema";

function getOrInitProtocol(): _Protocol {
  let protocol = _Protocol.load("1");
  if (!protocol) {
    protocol = new _Protocol("1");
    protocol.loans = [];
...
export function updateTotalWriteDowns(amount: BigInt): void {
  ...
  protocol.defaultRate =
    protocol.totalDrawdowns.isZero() || protocol.totalWritedowns.isZero()
      ? BigDecimal.zero()
      : protocol.totalWritedowns.divDecimal(
          protocol.totalDrawdowns.toBigDecimal()
        );
  protocol.save();
}
